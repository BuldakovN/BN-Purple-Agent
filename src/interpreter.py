"""
Python interpreter that executes code in an isolated subprocess.
Supports persistent sessions (variables survive across calls) and timeouts.
"""
...
def _run_session(working_dir: str, code_inq: Queue, result_outq: Queue, event_outq: Queue) -> None:
    ...
    while True:
        ...
        try:
            exec(compile(code, " ", "exec"), global_scope)
...
class Interpreter:
    ...
    def run(self, code: str, reset_session: bool = False) -> ExecutionResult:
        ...
        try:
            ...
            raise RuntimeError("Interpreter child process failed to start execution") from None
        ...
        while True:
            try:
                ...
                if elapsed > self.timeout:
                    logger.warning("Execution exceeded timeout of %ss", self.timeout)
